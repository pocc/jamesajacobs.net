...
export default function BookCard({ book, featured = false }: Props) {
    if (featured) {
        return (
            <div className="flex flex-col md:flex-row gap-8 items-start">
                {/* Book cover - real image or CSS placeholder */}
                {book.coverImage ? (
                    <div className="flex-shrink-0 w-48 h-64 rounded-lg shadow-xl overflow-hidden">
                        <img
                            src={book.coverImage}
    ...
    return (
        <div className="bg-white rounded-xl border border-surface-dark/50 p-6 flex flex-col h-full hover:shadow-md transition-shadow">
            {/* Mini cover - real image or CSS placeholder */}
            {book.coverImage ? (
